...
- **실전 중급**: SBC 크로스 컴파일·배포와 embedded-hal 드라이버 조합을 스스로 진단하고, 권한·버스·타이밍 실패를 분리하게 한다.
- **고급**: 레지스터·인터럽트·DMA·동시성, WCET/지연·전력, unsafe 경계와 데이터시트 근거까지 요구한다. 예제가 한 번 동작했다는 사실만으로 고급으로 판정하지 않는다.
