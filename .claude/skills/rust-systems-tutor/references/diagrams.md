...
아래 항목 중 하나라도 실패하면 OS 설명용 다이어그램은 통과가 아니다.

- **목적 적합성**: 호출 경로는 flowchart, 시간 순서는 sequence, 상태 변화는 state, 주소·버퍼 배치는 ASCII처럼 질문의 관계에 맞는 형식을 골랐는가?
- **경계 정확성**: 유저/커널, 프로세스/스레드, safe/unsafe, 호스트/장비 경계를 필요한 곳에 표시했는가?
- **방향과 시간**: 화살표가 제어·데이터·wake/return 중 무엇인지 읽을 수 있고, 요청과 반환 또는 block과 wake 순서가 뒤바뀌지 않았는가?
- **상태 완전성**: 시작·대기·성공뿐 아니라 질문에 중요한 오류, 취소, `EAGAIN`, page fault 같은 실패·재시도 경로가 있는가?
- **근거 대응**: 핵심 노드와 화살표를 문서, 소스, `strace`/로그/레지스터 관찰 중 하나에 대응시킬 수 있는가? 추론이면 추론이라고 표시했는가?
- **인지 부하**: 한 그림이 한 질문에 답하고 노드가 12개 이내인가? 범위를 넘으면 개요와 상세 그림으로 나눴는가?
- **접근성**: Mermaid가 렌더링되지 않아도 바로 아래 설명이나 ASCII 대안으로 핵심 흐름을 이해할 수 있는가?
- **학습 확인**: 학습자가 빈칸을 채우거나 변형 흐름을 직접 그려 이해를 입증할 수 있는가?

렌더링 성공은 의미 정확성을 보장하지 않는다. 문법 검사와 의미 검사를 별도로 하고, 관찰 결과와 충돌하면 다이어그램을 수정한다.
